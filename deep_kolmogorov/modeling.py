...
class DNN(BaseNet):
    ...
    def __init__(self, config):
        ...
        self.size_t, self.size_x, self.size_u = self.config["size_t_x_u"]
        self.size_input = self.size_t + self.size_x + self.size_u
...
class KolmogorovNet(torch.nn.Module):
    ...
    def forward(self, batch, train=True, onlynet=False):
        if onlynet:
            with torch.no_grad():
            ...
            if not train:
                y_pred = torch.maximum(y_pred, torch.nn.ReLU()(batch["K"]-batch["x"]))
            return y_pred
        else:
            with torch.no_grad():
                if train:
                    ...
                    y = batch["solution"]
                    y = torch.maximum(y, torch.nn.ReLU()(batch["K"]-batch["x"]))
                if hasattr(self.net, "No_normalization_and_flatten"):
                    tensor = self.pde.normalize_and_flatten(batch, No_normalization_and_flatten = True)
                else:
                    tensor = self.pde.normalize_and_flatten(batch, No_normalization_and_flatten = False)
