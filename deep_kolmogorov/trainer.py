...
class Data_Saved(Dataset):
    def __init__(self, path, n_batches=None, iter_all=True, num_ex = 2, cur_ex = 1):
    ...
    def __getitem__(self, idx):
        ...
        res = {k:v[n//self.num_ex * self.cur_ex : n//self.num_ex * (self.cur_ex + 1)] for k,v in data.items() if k != "solution"}
        res["solution"] = data["solution"][:, self.cur_ex:self.cur_ex+1]
        return res
...
class Trainer(tune.Trainable):
    ...
    def setup(self, config):
        ...
        pde_kwargs["payoff"] = config["payoff"]
        self.pde = PDES[config["pde"]](**pde_kwargs)
        self.pde.t, self.pde.dt = config["t"], config["dt"] # set up the parameter dt and t for pde
        ...
        self.test_loader = DataLoader(Data_Saved(os.path.join(self.data_path, "test"), config["n_test_batches"], num_ex=config["num_ex"],cur_ex=config["cur_ex"]), None, num_workers=0)
        self.val_loader = DataLoader(Data_Saved(os.path.join(self.data_path, "val"), config["n_test_batches"], num_ex=config["num_ex"],cur_ex=config["cur_ex"]), None, num_workers=0)
        # stats
...
def main(config, dt, i, num_ex, base_dir, payoff):
    if not config["mode"] == "default":
        config.update(HYPERCONFIGS[config["mode"]])
    config["dt"], config["t"], config["payoff"], config["num_ex"], config["cur_ex"] = dt, i*dt, payoff, num_ex, i # add parameter dt, t, payoff
    if config.get("sched"):
    ...
    ray.init(dashboard_host="0.0.0.0", num_gpus=torch.cuda.device_count(), ignore_reinit_error=True)
